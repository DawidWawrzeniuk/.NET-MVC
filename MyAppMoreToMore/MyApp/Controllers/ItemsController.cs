...
namespace MyAppMoreToMore.Controllers //Definiuje przestrzeń nazw dla kontrolerów w aplikacji MyApp
{
    public class ItemsController : Controller       //Definiuje klasę ItemsController, która dziedziczy po klasie Controller. Oznacza to, że ItemsController ma dostęp do funkcji i metod kontrolera w ASP.NET Core MVC
    {
        ...
        {
            var item = await _context.Items.Include(s => s.SerialNumber)               //await _context.Items.ToListAsync(); – tutaj ToListAsync() pobiera wszystkie rekordy z tabeli Items w bazie danych w sposób asynchroniczny, czyli nie blokując wątku (nie czekając bezczynnie na wyniki),		Metoda Include służy do załadowania powiązanych danych. W tym przypadku, gdy pobierasz przedmioty, jednocześnie ładujesz powiązane obiekty SerialNumber. Umożliwia to dostęp do informacji o numerze seryjnym dla każdego przedmiotu bez dodatkowego zapytania do bazy danych.
										   .Include(c=>c.Category)                     //ToListAsync(): Asynchroniczna metoda, która pobiera wszystkie elementy z Items i zwraca je jako listę. W tym przypadku wszystkie przedmioty oraz ich powiązane numery seryjne.
										   .Include(ic => ic.ItemClients)              //.Include(ic => ic.ItemClients) – Wskazuje Entity Framework, że dla każdego obiektu Item chcemy załadować powiązane obiekty ItemClients. Ładowanie tabeli pośredniczącej jest potrzebne, ponieważ przechowuje informacje o relacjach między Item a Client..ThenInclude(c => c.Client) – Dla każdego obiektu ItemClient załadowanego w poprzednim kroku chcemy również załadować powiązany obiekt Client. W ten sposób Entity Framework pobierze wszystkie Client powiązane z każdym Item przez tabelę pośredniczącą ItemClient.
        ...
        {
            ViewData["Categories"] = new SelectList(_context.Categories, "Id", "Name");     //do klucza "Categories" w słowniku ViewData dodajemy listę rozwijaną selectlist
                                                                                            //SelectList to specjalna klasa w ASP.NET MVC, która jest używana do tworzenia list rozwijanych (dropdown lists) w formularzach. Umożliwia wyświetlenie zbioru danych (np. z bazy danych) w formie listy, z której użytkownik może wybrać jedną wartość.
			return View();
        }
        ...
        public async Task<IActionResult> Create([Bind("Id, Name, Price, CategoryId")] Item item)    //async: Oznacza, że metoda jest asynchroniczna, co pozwala na nieblokujące operacje, takie jak zapisywanie danych do bazy.
